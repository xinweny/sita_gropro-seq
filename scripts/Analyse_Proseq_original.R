analyzeProseq <- function(samples, bgCov, geneCov, counts, agg, condition, batch, cond1, cond2){
  ...
  result = as.data.frame(results(ddsProseq, contrast=c("condition",cond2,cond1), alpha = 0.05))
...
plotMAProseq <- function(df, cutoff, label){
  ...
  if(label){
    ...
    gg <- ggplot(data=df, aes(x=log10(df$baseMean), y=log2FoldChange, label = label)) + theme_classic() + scale_shape_identity() + geom_point(alpha=1, size=2, shape=df$shape, fill=df$color) + geom_text_repel(size=3.5) +  scale_colour_manual(values=c('grey','red')) + ylim(-cutoff, cutoff) + theme(legend.position = "none") 
    print(gg)
  } else {
    gg <- ggplot(data=df, aes(x=log10(df$baseMean), y=log2FoldChange))  + theme_classic() + scale_shape_identity() + geom_point(alpha=1, size=2, shape=df$shape, fill=df$color) +  scale_colour_manual(values=c('grey','red')) + ylim(-cutoff, cutoff) + theme(legend.position = "none") 
    print(gg)
  }
